...
class AttPool(nn.Module):
    def __init__(self, config, const_k=None, const_q=None, prior_map=None):
        ...
        self.prior_map = prior_map.to(device)
...
class PaiAttPool(nn.Module):
    def __init__(self, config, const_k=None, const_q=None, prior_map=None):
        super(PaiAttPool, self).__init__()
        self.num_k = config['num_k']
        self.num_q = config['num_q']
        self.init = 'normal'
        self.prior_init = config['prior_init'] if 'prior_init' in config else False
        ...
        num_base = self.num_base if self.num_k > 128 else self.num_k
        self.key = Parameter(torch.Tensor(num_base, self.dim_att))
        self.mlp_1 = nn.Linear(mappingsize*2, num_base) if self.num_k > 128 else None
        # num_base = self.num_base if self.num_q > 128 else self.num_q
        self.query = Parameter(torch.Tensor(self.num_q, self.dim_att))
        # self.mlp_2 = nn.Linear(mappingsize*2, self.num_base) if self.num_q > 128 else None
        self.weight_prior = Parameter(torch.Tensor(2, 1))
        ...
        self.norm = lambda x: F.normalize(x, p=1, dim=-1)
        self.norm2 = lambda x: F.normalize(x, p=2, dim=-1)
    ...
    def get_score(self, key=None, query=None):        
        key_in = self.key if key is None else key
        query_in = self.query if query is None else query
        key_in, query_in = self.norm2(key_in), self.norm2(query_in)
        dist = torch.transpose(torch.matmul(key_in, torch.transpose(query_in, -2, -1)), -2, -1)
        score = dist
    ...
    def forward(self, x, t_vertex_1, key=None, query=None):
        
        key = torch.matmul(self.mlp_1(t_vertex_1), self.key) if self.num_k > 128 else self.key
        query = self.query
...
class PaiAttPool2(nn.Module):
    def __init__(self, config, const_k=None, const_q=None, prior_map=None):
        super(PaiAttPool2, self).__init__()
        self.num_k = config['num_k']
        self.num_q = config['num_q']
        self.prior_init = config['prior_init'] if 'prior_init' in config else False
        self.prior_coef = config['prior_coef'] if 'prior_coef' in config else 0
        ...
        key = torch.matmul(torch.transpose(const_q, -2, -1), const_k)
    ...
    def gen_map(self, score=None):  
        ...
        if self.top_k == 0:
            ...
            topk_norm = torch.zeros(self.num_q, self.num_k,
                            device=self.key.device).scatter_(dim=-1, index=idx, src=val_norm)
        topk = self.apply_prior_map(topk_norm)
        return topk

    def apply_prior_map(self, topk):
        denominator = torch.sum(self.weight_prior) \
            if torch.abs(torch.sum(self.weight_prior)) > self.epsilon else self.epsilon
        topk[-1, :], topk[:, -1], topk[-1, -1] = 0., 0., 1.
        topk = torch.matmul(torch.cat([self.prior_map.unsqueeze(-1), topk.unsqueeze(-1)], dim=-1),
...
class PaiAttPool3(nn.Module):
    def __init__(self, config, const_k=None, const_q=None, prior_map=None):
        super(PaiAttPool3, self).__init__()
        self.num_k = config['num_k']
        self.num_q = config['num_q']
        self.init = 'normal'
        self.prior_init = config['prior_init'] if 'prior_init' in config else False
        ...
        mappingsize = 64
        self.key = nn.Linear(2*mappingsize, self.dim_att) # Parameter(torch.Tensor(self.dim_att, mappingsize))
        self.query = nn.Linear(2*mappingsize, self.dim_att) # Parameter(torch.Tensor(self.dim_att, mappingsize))
        self.weight_prior = Parameter(torch.Tensor(2, 1))
        self.const_k = const_k
        self.const_q = const_q
        self.prior_map = prior_map.to(device)
    ...
    def get_score(self, t_vertex_1, t_vertex_2, key=None, query=None):        
        key_in = self.key(t_vertex_1).t() if key is None else key
        query_in = self.query(t_vertex_2).t() if query is None else query
    ...
    def gen_map(self, score):        
        if self.top_k == 0:
            ...
            if len(score.size()) == 2:
                topk_norm = torch.zeros(self.num_q, self.num_k,
                                        device=score.device).scatter_(dim=-1, index=idx, src=val_norm)
            elif len(score.size()) == 3:
                topk_norm = torch.zeros(score.size()[0], self.num_q, self.num_k,
                                        device=score.device).scatter_(dim=-1, index=idx, src=val_norm)
            else:
    ...
    def forward(self, x, t_vertex_1, t_vertex_2, key=None, query=None):
        pmap, _ = self.gen_map(self.get_score(t_vertex_1, t_vertex_2, key=key, query=query))
